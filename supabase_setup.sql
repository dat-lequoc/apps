...
-- Enable RLS for time_tracking table
alter table time_tracking enable row level security;

-- Create policy to allow users to view their own time tracking data
create policy "Users can view their own time tracking data"
  on time_tracking for select
  using ( auth.uid() = user_id );

-- Create policy to allow users to insert their own time tracking data
create policy "Users can insert their own time tracking data"
  on time_tracking for insert
  with check ( auth.uid() = user_id );

-- Create policy to allow users to update their own time tracking data
create policy "Users can update their own time tracking data"
  on time_tracking for update
  using ( auth.uid() = user_id );

-- Create policy to allow users to delete their own time tracking data
